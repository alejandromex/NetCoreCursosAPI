...
namespace Aplicacion.Cursos
{
    public class PaginacionCurso
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, PaginacionModel>
        {
            ...
            {
                this._paginacionRepositorio = paginacion;
            }
            public async Task<PaginacionModel> Handle(Ejecuta request, CancellationToken cancellationToken)
            {
                ...
                parametros["NombreCurso"] = request.Titulo;
                return await _paginacionRepositorio.devolverPaginacion(storeProcedure,request.NumeroPagina, request.CantidadElementos,parametros,ordenamiento);
            }
        }
    }
}